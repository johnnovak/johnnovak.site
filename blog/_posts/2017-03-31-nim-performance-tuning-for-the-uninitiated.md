...
for such tests.  But then how can we be ever certain that the compiler hasn't
...
output, namely the resulting binary. Luckily, we don't have to do exactly that
as most compilers can be instructed to emit post-optimisation assembly sources
that are directly used for generating the final binary.

While this might sound a little intimidating for non-assembly programmers
(which is probably at least 99.99% of all programmers in the world today), in
...
abysmal, about 1-2 millions of tests per second only! After much head
scratching and debugging it turned out that nim-glm was the culprit: the
vector component getter and setter methods were not inlined by the compiler.
After a few strategically placed [inline
pragmas](https://github.com/stavenko/nim-glm/commit/aebc0ee68f6d3ed5ccc4fcc89dd81716af708c6e)
the situation got better, but still a 10-fold performance degradation compared
to the C++ version:
...
component that heavily uses macros is just asking for pain.
...
(Chrome and Firefox was tested) yielded basically the same results.
...
benchmark, that's all languages other than C++ and Nim. While the performance
of C++ and Nim scale linearly with the size of the dataset, for the JITed
languages the performance is roughly a logarithmic function of the dataset
size, as summarised by the below table:
...
easily reproducible performance tests and run them periodically as it's very
easy to introduce quite severe performance degradations even with the most
innocent looking refactorings (e.g. think of the inlining problem after we
extracted the vector operations into a module). Without a systematic approach
to performance regression testing, problems like this can be quite frustrating
and time consuming to locate and fix. Also, there's nothing like inspecting
the actual assembly output; that's the only foolproof way to catch the
compiler red-handed at optimising away your test code.

But surely with so much power to spare on modern CPUs, all this micro-optimisation is just waste of time, right? After all, hardware is cheap and programmers are expensive. Well, that depends. While I agree that performance is probably not so critical for the majority of applications and for those apps it makes sense (commercially, at least) to optimise for progammer time, when it matters 
...
