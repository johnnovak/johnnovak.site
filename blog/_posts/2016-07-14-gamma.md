...
it comes to gamma-correctness. Note how antialising in γ=2.2 space results in
overly dark "smoothing pixels" (right image); the text appear too heavy,
almost as if it was bold. Running the algorithm in linear space produces much
better results (left image), although in this case the font looks a bit too
thin. Interestingly, Photoshop antialiases text using γ=1.42 by default,
and this indeed seems to yield the most natural results (middle image).
I don't know the explanation for this, but the take away point is that
antialaising in sRGB space is almost never what you want.
...
physically-based rendering (PBR). For example, in the real world, the
contribution of each light source to a scene will add up linearly. By doing
lighting calculations in the non-linear sRGB space, we will be violating this
assumption and therefore everything will look just completely wrong (e.g.
blown highlights, weird hue and saturation shifts as the light intensity
increases, quadratic light falloff will not be quadratic anymore etc.)

On the left quite natural looking image, the rendering took place in linear
space and then the image was converted to sRGB before it was saved to disk.
This last conversion step was omitted from the right image (the one sporting
the suspicious CGI look), so effectively we were writing the linear
framebuffer data to the file unaltered, *which then gets interpreted as sRGB*
by the image viewers. This will make the image look too dark, of course, so
the next thing I did was increase the light intensities so the brownish ground
has about the same shade as in the original image. Well, but that made
everything too contrasty and oversaturated...  I just stopped there, but the
next "fixing" attempt could be desaturating the colours of the objects and
maybe increase the intensity of the fill light.  However, it can easily seen
that this would be a losing game; one can "tweak" the various parameters so
that the final image will somewhat resemble a realistic looking scene, but we
just cannot fudge realistic linear light behaviour while working in
a non-linear space. The tweaked results will always be inferior to physically
correct rendering, and once the lighting in the scene changes, everything will
just fall apart like a house of cards. Save yourself the pain, just don't do
it.
...
Incorrect gamma handling in 3D rendering is one of the main culprits
responsible for the "fake plasticky CGI look" in some (mostly older) games. As
illustrated on the image below, rendering realistic looking human skin is
almost impossible with a gamma-incorrect workflow; the highlights will just
never look right.
