...
Don't feel bad if you have answered with a "yes" to most! I would have given
...
don't get gamma completaly right (or at all) and produce technically incorrect
results.
...
category, I did just check). Some books mention gamma correction in passing
...
and phones are generally quite inaccurate compared to monitors, so try
...
physically-linear, we would be losing precision on dark shades while
being relatively unnecessarily precise on light shades, as shown before.
...
about 22%! Of course, if we're talking about *perceptual* light intensity,
then RGB(128, 128, 128) *appears* to be half as bright as RGB(255, 255, 255)
to the human eye! If you find this confusing, reflect a bit on it or read this
whole section again, because it's crucial to have a clear understanding of what
has been discussed so far before progressing any further (trust me, it will only
get messier).
...
viewers and browsers will interpret them as sRGB by convention.
...
The problem will happen in the moment we started running any image processing
algorithms on our sRGB data directly. Remember, gamma encoding is a non-linear
transformation and sRGB encoding is basically just a funky way of doing gamma
encoding of around γ=2.2 or so. Virtually all image processing algorithms you
can find in computer graphics texts will assume *linearly-encoded light
intensities*, so this means that all our processing
will be subtly---or in some cases quite obviously---wrong! This includes
resizing, blurring, compositing, interpolating between pixel values,
antialiasing and so on.


### Hey, what about CRT displays?
...
That's exactly what I'll do in this section; I'll present the most common
examples how running image processing algorithms directly on sRGB data would
manifest in gamma-incorrect results. These example images are also useful for
...
differences might be less noticeable or even negligible (however, they are
still there, and image processing programs should work correctly for all
possible inputs, not just okayish for 65.23% of all possible images...).
...
yields a much darker and sometimes more saturated looking images.
...
wall; the colours would mix on the wall as in the linear-space gradients).

{% include image.html name="gradient.png" caption="Figure 7 &mdash; For each gradient-pair, the top gradient is a linear interpolation between two colours in linear space, then the result converted to sRGB (gamma-correct). For the bottom gradients, we interpolated between the exact same colours but directly on the sRGB values (gamma-incorrect)." width="80%" captionAlign="center" %}
...
a weird darkish transition band with certain vivid colour combinations.
...
up. Let's examine some coloured rectangles first.  As expected, the
gamma-correct image on the left mimics how light would behave in real life,
while the sRGB space blending on the right exhibits some weird colour and
brigthness shifts.

{% include image.html name="opacity.png" caption="Figure 9 &mdash; Effects of gamma-incorrect alpha blending. For each vertical bar pair, the top one is drawn with 100% opacity and bottom one with 50%. The left one is the gamma-correct image. The test was done in Photoshop CS6, similarly to figure X." %}
...
while on the right image there's a noticable greenish cast over the whole
image---the skin now looks slightly green and the flowers and her lips have also
lost their original hues and have turned into a muddy green.

Again, the right image might be an effect you like, but it's not how accurate
alpha compositing should work.

{% include image.html nameSmall="blending.jpg" name="blending.jpg" caption="Figure 10 &mdash; Blending of two photographic images. The two original images from the top row are laid on top of each other in the bottom row, the blueish image having 60% opacity and the lady with the flowers 100%. The left image is the gamma-correct one (click to enlarge to see the details better). Tested with Photoshop CS6." %}

### Image resizing

{% include image.html name="resize-large.png" caption="Figure 11 &mdash; Black and white checkerboard pixel pattern frequently used in simple gamma calibration programs. The right image is just showing the pattern at 400% magnification." captionAlign="center" captionWidth="70%" width="220px" %}

{% include image.html name="resize.png" caption="Figure 12 &mdash; Effects of gamma-incorrect image resizing. A is the pixel checkerboard pattern, B the gamma-incorrect result of resizing the image in the sRGB space (Photoshop CS6 in 8-bit RGB mode), and C the gamma-correct result of converting the image to linear space before resizing, then back to sRGB (Photoshop CS6 in 32-bit RGB mode)." captionAlign="center" captionWidth="80%" width="316px" %}
...
{% include image.html name="lightfalloff.png" alt="TODO" caption="Figure 14 &mdash; Effects of gamma-incorrect rendering on a diffuse sphere. (Image from the <a href=\"https://docs.unity3d.com/Manual/LinearLighting.html\">Unity3D manual</a>.)" width="70%" captionAlign="center" %}

{% include image.html name="lineargammahead.png" alt="TODO" caption="Figure 15 &mdash; Effects of gamma-incorrect rendering on a human head. (Image from the <a href=\"https://docs.unity3d.com/Manual/LinearLighting.html\">Unity3D manual</a>.)" width="90%" captionAlign="center" %}
